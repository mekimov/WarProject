...
public class Turn
{
    ...
    public void Begin()
    {
        Game.Instance.BattleController.StartCoroutine(TurnRoutine());
    }
    private IEnumerator TurnRoutine()
    {
        foreach (Unit unit in units)
        {
            var c = Game.Instance.BattleController.StartCoroutine(UnitTurnRoutine(unit));
    ...
    private IEnumerator UnitTurnRoutine(Unit unit)
    {
        ...
        {
            Game.Instance.BattleController.StartCoroutine(FightRoutine(unit, unit.attackTarget));
        }
        units.Remove(unit);
    }
    ...
    private IEnumerator FightRoutine(Unit unit1, Unit unit2)
    {
        ...
        Game.Instance.BattleController.StopCoroutine(r1);
        var r2 = GetUnitTurnInfo(unit2).turnRoutine;
        Game.Instance.BattleController.StopCoroutine(r2);
        Unit unitFirst;
        Unit unitSecond;
